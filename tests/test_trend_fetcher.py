...
class TestTrendFetcherAPI:
    ...
    def test_trend_alert_field_types(self):
        ...
        if len(alerts) > 0:
            ...
            datetime.fromisoformat(alert["window_start"].replace("Z", "+00:00"))  # Valid ISO 8601
            
            # window_end: string (ISO 8601)
            assert isinstance(alert["window_end"], str), \
                "window_end must be a string"
            datetime.fromisoformat(alert["window_end"].replace("Z", "+00:00"))  # Valid ISO 8601
            
            # created_at: string (ISO 8601)
            assert isinstance(alert["created_at"], str), \
                "created_at must be a string"
            datetime.fromisoformat(alert["created_at"].replace("Z", "+00:00"))  # Valid ISO 8601
    ...
    def test_trend_alert_time_window_ordering(self):
        ...
        if len(alerts) > 0:
            ...
            assert window_end <= created_at, \
                "window_end must be <= created_at"
    ...
    def test_trend_alert_json_serializable(self):
        """Test that TrendAlert is JSON serializable (required for MCP/API transport)."""
        import json
        ...
        if len(alerts) > 0:
            ...
            assert isinstance(json_str, str), \
                "TrendAlert must be JSON serializable"
    ...
    def test_trend_alert_matches_api_contract_example(self):
        """Test that TrendAlert structure matches the expected format from API contract."""
        agent_id = str(uuid.uuid4())
        resource_uris = ["mcp://news/ethiopia/fashion/trends"]
